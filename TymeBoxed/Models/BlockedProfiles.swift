...
class BlockedProfiles: Identifiable {
  ...
  /// Stored as `Int` (not `UInt32`) — SwiftData’s persistent store does not support unsigned integer attributes.
  var reminderTimeInSeconds: Int?
  var enableBreaks: Bool = false
  var breakTimeInMinutes: Int = 15
  var enableStrictMode: Bool = true
  ...
  /// Incremented on each successful push for CloudKit last-write-wins.
  var syncVersion: Int = 0

  /// True when the profile was synced from another device and needs local Screen Time selection.
  var needsAppSelection: Bool = false

  /// Schema version for forward-compatible sync (Tyme Boxed uses `1`).
  var profileSchemaVersion: Int = 1

  /// When a schedule-driven session was last manually stopped (optional cross-device hint).
  var scheduleLastStoppedAt: Date?

  @Relationship(deleteRule: .cascade, inverse: \BlockedProfileSession.blockedProfile)
  var sessions: [BlockedProfileSession] = []

  var isNewerSchemaVersion: Bool {
    profileSchemaVersion > TymeBoxedSyncSchema.currentProfileSchemaVersion
  }
  ...
  static func updateProfile(
    ...
    if let needsAppSelection {
      profile.needsAppSelection = needsAppSelection
    }
  ...
  static func deleteProfile(
    ...
    // Resolve attribute faults before deletion to prevent "detached from context" crash
    _ = profile.selectedActivity
    _ = profile.schedule
    _ = profile.strategyData
    _ = profile.domains

    // First end any active sessions and resolve session faults
    for session in profile.sessions {
      _ = session.blockedProfile
