          ...
          Button(action: {
            ...
            HStack {
              ...
              Text(resendCountdown > 0 ? "Resend code in \(resendCountdown)s" : "Resend code")
                ...
                .foregroundColor(
                  resendCountdown > 0 || authManager.isLoading
                    ? .secondary : .accentColor
                )
            }
          }
          .disabled(resendCountdown > 0 || authManager.isLoading)
          .padding(.bottom, 32)
    ...
    return Text(digit)
      ...
      .background(
        RoundedRectangle(cornerRadius: 12)
          ...
          .overlay(
            RoundedRectangle(cornerRadius: 12)
              .stroke(shouldShowBorder ? Color.primary : Color.clear, lineWidth: 2)
          )
      )
      .animation(.easeInOut(duration: 0.2), value: currentBoxIndex)
  }
