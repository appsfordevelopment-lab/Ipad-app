...
struct BlockedProfileView: View {
  ...
  @State private var enableStrictMode: Bool = true
  ...
  /// After a successful on-screen resync, hide the banner (DeviceActivityCenter is not observable).
  @State private var scheduleOutOfSyncBannerSuppressed = false
  ...
  private var shouldShowScheduleOutOfSyncWarning: Bool {
    guard let profile else { return false }
    if scheduleOutOfSyncBannerSuppressed { return false }
    return profile.scheduleIsOutOfSync
  }
  ...
  init(profile: BlockedProfiles? = nil) {
    ...
    _enableStrictMode = State(
      initialValue: profile?.enableStrictMode ?? true
    )
  ...
  var body: some View {
    NavigationStack {
      Form {
        ...
        if shouldShowScheduleOutOfSyncWarning {
          Section {
        ...
        Section {
          HStack(spacing: 12) {
            Image(systemName: profileSyncManager.isEnabled ? "checkmark.icloud.fill" : "icloud.slash.fill")
              .foregroundStyle(
                profileSyncManager.isEnabled ? themeManager.themeColor : Color.secondary
              )
              .font(.title3)

            VStack(alignment: .leading, spacing: 2) {
              Text(
                profileSyncManager.isEnabled
                  ? "This profile will sync to your other devices"
                  : "iCloud sync is off"
              )
              .font(.subheadline)
              Text(
                profileSyncManager.isEnabled
                  ? "All profiles sync when Device Sync is enabled in Settings."
                  : "Enable Device Sync in Settings to sync profiles across your devices."
              )
              .font(.caption)
...
//         }
      ...
      .navigationTitle(isEditing ? "Profile Details" : "Create New Profile")
      .toolbar {
      ...
      .onChange(of: profile?.id) { _, _ in
        scheduleOutOfSyncBannerSuppressed = false
      }
      .onAppear {
        attemptAutomaticScheduleResync()
      }
  ...
  /// Re-register Device Activity when the banner would show; avoids stale UI (center is not observable).
  private func attemptAutomaticScheduleResync() {
    guard let profile, profile.scheduleIsOutOfSync, !isBlocking else { return }
    DeviceActivityCenterUtil.scheduleTimerActivity(for: profile)
    if DeviceActivityCenterUtil.getActiveScheduleTimerActivity(for: profile) != nil {
      scheduleOutOfSyncBannerSuppressed = true
    }
  }
  ...
  private func saveProfile() {
    do {
      ...
      if let existingProfile = profile {
        ...
        updatedProfile.needsAppSelection = false
        ...
        newProfile.needsAppSelection = false
